...
def main():
    ...
    review_df = spark.read.parquet("./data/parquet_format/yelp_academic_dataset_review.parquet")
    
    # Filter out the necessary columns
    business_df = business_df.select("business_id", "name", "review_count", "attributes", "categories")
    ...
    # Join dataframes
    joined_df = business_df.join(review_df, "business_id", "inner").select("name", "stars", "text")

    # Drop rows where any of the values are null
    joined_df = joined_df.filter(joined_df.name.isNotNull() & joined_df.stars.isNotNull() & joined_df.text.isNotNull())
